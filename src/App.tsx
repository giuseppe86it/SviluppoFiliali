...
import { RapidInspectionPage } from './pages/RapidInspectionPage'
...
export default function App(){const hash=useHash();useEffect(()=>{navigator.storage?.persist?.().catch(()=>{})},[]);const[unlocked,setUnlocked]=useState(()=>!getPinHash()||sessionStorage.getItem('sf-unlocked')==='1');if(!unlocked)return <LockScreen onUnlock={()=>setUnlocked(true)}/>;const clean=hash.slice(2).split('?')[0];const parts=clean.split('/').filter(Boolean);const route=parts[0]||'home';if(route==='home')return <HomePage/>;if(route==='new')return <NewInspectionPage/>;if(route==='rapid')return <RapidInspectionPage id={parts[1]}/>;if(route==='inspection')return <InspectionPage id={parts[1]} sectionId={parts[2]||'general'}/>;if(route==='archive')return <ArchivePage/>;if(route==='trash')return <TrashPage/>;if(route==='map')return <MapPage/>;if(route==='backup')return <BackupPage/>;if(route==='settings')return <SettingsPage/>;if(route==='share')return <SharePage id={parts[1]}/>;if(route==='more')return <MorePage/>;if(route==='about')return <AboutPage/>;return <HomePage/>}
