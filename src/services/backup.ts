...
const MAGIC='SFBK1'
async function deriveKey(password:string,salt:Uint8Array){const base=await crypto.subtle.importKey('raw',new TextEncoder().encode(password),'PBKDF2',false,['deriveKey']);return crypto.subtle.deriveKey({name:'PBKDF2',salt,iterations:180000,hash:'SHA-256'},base,{name:'AES-GCM',length:256},false,['encrypt','decrypt'])}
async function encryptBlob(blob:Blob,password:string){const salt=crypto.getRandomValues(new Uint8Array(16)),iv=crypto.getRandomValues(new Uint8Array(12)),key=await deriveKey(password,salt),plain=await blob.arrayBuffer(),cipher=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,plain);return new Blob([MAGIC,salt,iv,cipher],{type:'application/octet-stream'})}
async function decryptBlob(blob:Blob,password:string){const magic=await blob.slice(0,5).text();if(magic!==MAGIC)return blob;if(!password)throw new Error('PASSWORD_REQUIRED');const salt=new Uint8Array(await blob.slice(5,21).arrayBuffer()),iv=new Uint8Array(await blob.slice(21,33).arrayBuffer()),cipher=await blob.slice(33).arrayBuffer(),key=await deriveKey(password,salt);try{const plain=await crypto.subtle.decrypt({name:'AES-GCM',iv},key,cipher);return new Blob([plain],{type:'application/zip'})}catch{throw new Error('Password backup non corretta o file danneggiato')}}
